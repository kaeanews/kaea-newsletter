...
LIST_MAX_PAGES = 12      # 안전 상한 (최대 1,200건까지 훑는다)
...
def get_sent_emails(since=""):
    ...
    for page in range(LIST_MAX_PAGES):
        data = _api_get("/emails", params={"offset": page * LIST_PAGE, "limit": LIST_PAGE})
        if data is None:
            if page == 0:
    ...
    if len(items) >= LIST_PAGE:
        print(f"  목록 {len(items)}건 조회 (여러 페이지)")
    ...
    if since:
        targets = [e for e in targets if (e.get("sentTime") or "")[:10] >= since]
    ...
    if sent_all and not before_since:
        ...
        for e in sorted(sent_all, key=lambda x: x.get("sentTime") or "", reverse=True)[:15]:
        ...
        print("     시작 날짜를 앞당기거나 비워서 다시 실행하세요.")
    elif not sent_all:
