...
def get_sent_emails():
    ...
    targets = [
        e for e in items
        if e.get("status") == STATUS_SENT and SUBJECT_FILTER in (e.get("subject") or "")
    ]
    targets.sort(key=lambda e: e.get("sentTime") or "", reverse=True)
    print(f"이메일 목록: 전체 {len(items)}건 → 발송완료+제목일치 {len(targets)}건")
...
def main():
    ...
    if "--email" in args:
        ...
        if not emails:
            print("[안내] 새로 변환할 메일이 없습니다. (모두 변환됨)")
            return
    else:
