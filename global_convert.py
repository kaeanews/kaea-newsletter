...
<style>
  * {{ box-sizing: border-box; }}
  ...
  .top {{ background:#1e3a5f; color:#fff; padding:28px 32px; }}
  .top .brand {{ font-size:12px; letter-spacing:2px; opacity:.75; }}
  .top h1 {{ margin:8px 0 4px; font-size:24px; font-weight:700; }}
  .top .sub {{ font-size:13px; opacity:.8; }}
  .back {{ padding:14px 32px; background:#eef2f7; border-bottom:1px solid #dde3ea; }}
  .back a {{ color:#1e3a5f; text-decoration:none; font-size:13px; font-weight:600; }}
  .back a:hover {{ text-decoration:underline; }}
  .cat {{ background:#3b5378; color:#fff; padding:11px 32px;
         font-size:15px; font-weight:700; }}
  .item {{ padding:20px 32px; border-bottom:1px solid #eef0f3; }}
  .item:last-child {{ border-bottom:none; }}
  .t {{ font-size:16px; font-weight:700; margin-bottom:7px; line-height:1.5; }}
  .t a {{ color:#1e3a5f; text-decoration:none; }}
  .t a:hover {{ text-decoration:underline; }}
  .meta {{ font-size:12px; color:#6b7280; margin-bottom:8px; }}
  .meta .badge {{ display:inline-block; background:#eef2f7; color:#3b5378;
                 padding:2px 8px; border-radius:3px; margin-right:6px; font-weight:600; }}
  .s {{ font-size:14px; color:#374151; }}
  .link {{ margin-top:8px; font-size:12px; }}
  .link a {{ color:#2563eb; text-decoration:none; word-break:break-all; }}
  .link a:hover {{ text-decoration:underline; }}
  .foot {{ padding:22px 32px; text-align:center; color:#9aa3af; font-size:12px;
          background:#f9fafb; }}
