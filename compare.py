...
# ⚠️ 0.65 로 두면 같은 사안을 다룬 '다른 기사'까지 합쳐진다.
#    실측: "5등급 지원 올해 끝..12월 5일까지" ↔ "5등급 지원 올해 종료⋯춘천시 독려" = 0.67
#    반면 진짜 같은 기사는 0.85 이상으로 나온다.
#      "서산시, ..." ↔ "충남 서산시, ..." = 0.96
SIMILARITY = 0.85

# 제목이 완전히 포함될 때도 길이가 너무 차이 나면 다른 기사로 본다.
# (짧은 제목이 긴 제목에 우연히 포함되는 경우 방지)
CONTAIN_LEN_RATIO = 0.7
...
def same_article(a, b):
    """두 기사가 같은 기사인지 판정.
    URL이 같으면 확실히 같은 기사. 아니면 제목으로 판정하되,
    같은 사안의 다른 기사를 합치지 않도록 기준을 엄격히 둔다."""
    ua, ub = (a.get("url") or "").strip(), (b.get("url") or "").strip()
    if ua and ub:
        if ua == ub:
            return True
        # URL이 서로 다르면 다른 언론사의 별개 기사일 가능성이 크다.
        # 제목이 거의 같을 때만 인정한다.
        na, nb = norm_title(a.get("title", "")), norm_title(b.get("title", ""))
        if not na or not nb:
            return False
        return na == nb or difflib.SequenceMatcher(None, na, nb).ratio() >= SIMILARITY
    ...
    if na == nb:
        return True
    # 포함 관계는 길이가 비슷할 때만 인정
    if na in nb or nb in na:
        short, long_ = (na, nb) if len(na) <= len(nb) else (nb, na)
        return len(short) / len(long_) >= CONTAIN_LEN_RATIO
...
# ── 비교 ───────────────────────────────────────────────────────────
def load_dropped_and_blocked(folder):
    """놓친 기사의 원인을 가리기 위해, 우리가 '버린 기록'을 모은다.
      blocked : AI 심사에서 차단된 기사 (사유 있음)
      dropped : 정원에서 밀려난 후보 (제목만)
    이 둘에도 없으면 애초에 검색에 안 잡힌 것이다."""
    blocked, dropped = [], []
    for path in sorted(glob.glob(os.path.join(folder, "*.json"))):
        try:
            with open(path, "r", encoding="utf-8") as f:
            ...
            continue
        blocked.extend(doc.get("blocked") or [])
        dropped.extend(doc.get("dropped") or [])
    return blocked, dropped


def diagnose(article, blocked, dropped):
    """놓친 기사 1건의 원인을 판정."""
    b = next((x for x in blocked if same_article(article, x)), None)
    if b:
        return "차단", f"{b.get('reason','')} · {b.get('detail','')}", b.get("category", "")
    d = next((x for x in dropped if same_article(article, x)), None)
    if d:
        return "정원컷", "후보에는 있었으나 선별에서 밀림", d.get("category", "")
    return "수집안됨", "검색 결과에 없음", ""
...
def compare_one(pubdate, truth, mine, blocked=(), dropped=()):
    ...
    missed_diag = [(a,) + diagnose(a, blocked, dropped) for a in missed]
...
def render(reports):
    ...
    kinds = Counter(k for r in reports for _, k, _, _ in r["missed_diag"])
    if kinds:
        add("  놓친 원인 분포")
        label = {"차단": "🚫 AI가 차단      → filter.py 판정 기준 조정",
                 "정원컷": "✂️ 선별에서 밀림  → 정원 상향 / 1차 선별 개선",
                 "수집안됨": "🔍 검색에 없음    → keywords.py 검색어 추가"}
        for k, v in kinds.most_common():
            add(f"    {v:>3}건  {label.get(k, k)}")
        add("")
    if tot_dup:
    ...
    for r in reports:
        ...
        same_file = [(k, e) for k, e in r["dups"] if k.get("file") == e.get("file")]
        cross_file = [(k, e) for k, e in r["dups"] if k.get("file") != e.get("file")]
        if same_file:
            add("")
            add("  🔁 같은 기사가 여러 카테고리에 실림 (수정 필요)")
            for keep, extra_a in same_file:
                add(f"     · {keep['title'][:46]}")
                add(f"         {keep['cat']}  +  {extra_a['cat']}")
        if cross_file:
            add("")
            add("  ℹ️ 다른 날 파일에도 같은 기사 (참고)")
            for keep, extra_a in cross_file:
                add(f"     · {keep['title'][:46]}")
                add(f"         {keep['file']} [{keep['cat'][:18]}]  /  {extra_a['file']} [{extra_a['cat'][:18]}]")

        if r["missed"]:
            add("")
            add(f"  ❌ 놓친 기사 ({len(r['missed'])}건) — 담당자는 실었는데 우리는 없음")
            icon = {"차단": "🚫", "정원컷": "✂️", "수집안됨": "🔍"}
            for a, kind, detail, ourcat in r["missed_diag"]:
                srcname = f"/{a['source']}" if a.get("source") else ""
                add(f"     · [{a.get('date','')}{srcname}] {a['title'][:48]}")
                add(f"         {icon.get(kind,'?')} {kind}: {detail}")
                add(f"         담당자 분류: {a['cat']}" + (f" / 우리 분류: {ourcat}" if ourcat else ""))
...
def main():
    ...
    blocked, dropped = load_dropped_and_blocked(DATA_DIR)
    ...
    for d in sorted(dates):
        ...
        reports.append(compare_one(d, truth, mine, blocked, dropped))
