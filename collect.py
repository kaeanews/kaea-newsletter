...
def _search_once(query, sort_mode):
    """네이버 뉴스 API 1회 호출 → 원본 items 리스트."""
    headers = {
        "X-Naver-Client-Id": CLIENT_ID,
        "X-Naver-Client-Secret": CLIENT_SECRET,
    }
    params = {"query": query, "display": DISPLAY, "sort": sort_mode}
    try:
        resp = requests.get(NAVER_NEWS_URL, headers=headers, params=params, timeout=10)
    except requests.RequestException as e:
        print(f"    [요청 실패] '{query}'({sort_mode}): {e}")
        return []
    if resp.status_code != 200:
        print(f"    [응답 오류] '{query}'({sort_mode}): HTTP {resp.status_code}")
        return []
