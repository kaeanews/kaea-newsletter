...
CATEGORY_TOPICS = {
    "한국자동차환경협회 뉴스":
        "한국자동차환경협회(자동차환경협회, AEA)가 기사에 직접 언급된 뉴스. "
        "협회의 활동·사업·발표·행사·인사, 협회가 주관하거나 참여한 사업",
    "상위기관 뉴스":
        "정부·공공기관(기후에너지환경부 등)의 자동차 환경 관련 정책·제도·법령·사업",
    "자동차 LCA 사업 뉴스":
        "협회가 수행하는 자동차 온실가스 전과정평가(LCA) 사업 관련 뉴스. "
        "LCA(Life Cycle Assessment)는 원료 채취부터 폐기까지 전 과정의 환경영향을 평가하는 것. "
        "자동차·배터리의 전과정평가·탄소발자국 산정과 검증체계 구축, "
        "ESG·지속가능성 공시 제도, 배터리 여권, 탄소국경조정제도(CBAM), "
        "배터리 재활용·순환경제 관련 제도와 사업이 해당됨",
    "배출 저감사업 뉴스":
        "협회가 수행하는 배출가스 저감 관련 사업 분야. "
        "DPF(매연저감장치) 부착, PM-NOx 저감장치, LPG 엔진개조, "
        "건설기계 DPF 부착·엔진교체, 저감장치 클리닝·점검·반납, 요소수 보충지원, "
        "조기폐차 사업(4·5등급 경유차, 도로용 3종 건설기계, 지게차·굴착기), "
        "운행차 배출가스 검사·결함확인검사",
    "전기·수소차 뉴스 - 협회 사업 관련 뉴스":
        "협회가 수행하는 전기차·수소차 분야 사업과 관련된 뉴스. "
        "협회 이름이 기사에 없어도 사업 내용이 맞으면 해당됨. "
        "① 전기차 공공급속충전시설 설치·운영·관리 "
        "② 전기차 충전시설 설치·운영 "
        "③ 지역별 무공해차 전환 브랜드사업 "
        "④ 이동식(이동형) 전기차 충전서비스 "
        "⑤ 전기화물차 무선충전시설 구축 "
        "⑥ 수소전기자동차 충전소 보급·운영·관리 "
        "⑦ 한국형 무공해차 전환100(K-EV100) 등 무공해차 전환 사업 "
        "⑧ 전기·수소 건설기계/지게차, 전기·수소 차량 개조 "
        "충전 인프라의 구축·운영·보조금·요금·관리도 포함",
    "전기·수소차 뉴스 - 업계 동향":
    ...
    "기타 뉴스":
        ...
        "미래형 모빌리티 인재교육, 친환경운전문화 확산 등 협회의 그 밖의 사업도 포함",
...
_GUIDE_TEMPLATE = """당신은 한국자동차환경협회가 회원사에게 보내는 뉴스 웹진의 편집자입니다.
...
[협회 관련 카테고리 구분 규칙] ★ 중요
'한국자동차환경협회 뉴스'와 '전기·수소차 뉴스 - 협회 사업 관련 뉴스'는 다음으로 가릅니다.
  · 기사에 '한국자동차환경협회(자동차환경협회, AEA)'가 직접 언급됨
      → '한국자동차환경협회 뉴스'에 해당
  · 협회 이름은 없지만, 협회가 수행하는 전기차·수소차 사업 내용과 관련됨
      → '전기·수소차 뉴스 - 협회 사업 관련 뉴스'에 해당
  ※ 즉 '협회 사업 관련 뉴스'는 협회 이름이 없어도 관련 있음(true)으로 판단하세요.
     지자체·기업·정부가 수행하는 일이라도 사업 내용이 협회 사업과 같으면 해당됩니다.
     예) "OO시, 공공급속충전기 30기 신규 설치" → 협회 이름 없어도 관련 있음
     예) "OO기업, 이동형 전기차 충전서비스 개시" → 협회 이름 없어도 관련 있음
     예) "물류센터에 전기화물차 무선충전시설 실증" → 관련 있음
     예) "지자체, 무공해차 전환 브랜드사업 참여기업 모집" → 관련 있음
...
# 카테고리 주제표는 CATEGORY_TOPICS 한 곳에서만 관리하고, 여기서 조립해 넣는다.
# (양쪽에 따로 적어두면 한쪽만 고쳤을 때 기준이 어긋난다)
_TOPIC_TABLE = "\n".join(f"  · {k}\n      {v}" for k, v in CATEGORY_TOPICS.items())
JUDGE_GUIDE = _GUIDE_TEMPLATE.replace("@@TOPICS@@", _TOPIC_TABLE)
...
def _extract_json(text):
    """응답에서 첫 번째 완전한 JSON 객체만 뽑아낸다.
    모델이 JSON 뒤에 설명을 덧붙여도('Extra data' 오류) 안전하게 파싱하기 위함.
    문자열 안의 중괄호·이스케이프를 구분해 중괄호 짝을 센다."""
    text = text.strip()
    if text.startswith("```"):
...
위 기사를 판단해 JSON 객체 하나만 출력하세요. 설명·인사말·코드블록 없이 JSON만."""
    ...
    try:
        resp = client.messages.create(
            ...
            messages=[
                ...
                # prefill: 응답을 '{' 부터 시작하게 강제해 잡담·설명이 앞에 붙는 것을 막는다
        ...
        if raw:
            print(f"      응답 미리보기: {raw[:120].replace(chr(10), ' | ')}")
...
# ── 카테고리 간 중복 제거 ──────────────────────────────────────────
# 같은 기사가 여러 카테고리 검색어에 걸려 두 곳에 실리는 일이 있었다.
#   실측: "배출가스 5등급 지원"  → 배출 저감사업 + 한국자동차환경협회 (URL 동일)
#         "쏘카 제주 자율주행"   → 협회 사업 관련 + 기타 뉴스        (URL 동일)
# 기존 중복 제거는 '카테고리 안에서만' 작동해 이걸 못 잡았다.
#
# 어느 카테고리에 남길지는 아래 순서로 정한다.
# 구체적인 주제를 다루는 카테고리를 앞에 두고, 포괄적인 '기타'를 맨 뒤에 둔다.
# (웹진 표시 순서와는 별개다. 여기는 '누가 우선권을 갖느냐'만 정한다)
DEDUP_PRIORITY = [
    "한국자동차환경협회 뉴스",                    # 협회명이 직접 나온 기사
    "자동차 LCA 사업 뉴스",                      # 전과정평가 등 특정 사업
    "배출 저감사업 뉴스",                        # 조기폐차·DPF 등 특정 사업
    "전기·수소차 뉴스 - 협회 사업 관련 뉴스",       # 충전 인프라 등 특정 사업
    "상위기관 뉴스",                            # 정부·기관 정책
    "회원사 뉴스",                              # 특정 기업 소식
    "전기·수소차 뉴스 - 업계 동향",               # 업계 일반
    "기타 뉴스",                                # 가장 포괄적
]

_DEDUP_BRACKET = re.compile(r"[\[\(【][^\]\)】]*[\]\)】]")
_DEDUP_NONWORD = re.compile(r"[^가-힣a-zA-Z0-9]")
DEDUP_SIMILARITY = 0.85


def _norm(t):
    return _DEDUP_NONWORD.sub("", _DEDUP_BRACKET.sub("", t or "")).lower()


def _is_same(a, b):
    """같은 기사인지 판정 (AI 호출 없이 URL·제목으로만)."""
    ...
    if na == nb:
        return True
    return difflib.SequenceMatcher(None, na, nb).ratio() >= DEDUP_SIMILARITY


def dedupe_across_categories(daily):
    """카테고리 간 중복 제거. 우선순위가 앞선 카테고리에만 남긴다.
    반환: (정리된 daily, 제거 내역)"""
    order = ([c for c in DEDUP_PRIORITY if c in daily]
             + [c for c in daily if c not in DEDUP_PRIORITY])
    seen = []          # [(기사, 남긴 카테고리)]
    removed = []
    for cat in order:
        kept = []
        for art in daily.get(cat, []):
            hit = next((s for s in seen if _is_same(art, s[0])), None)
            if hit:
                removed.append((art.get("title", ""), cat, hit[1]))
                continue
            seen.append((art, cat))
            kept.append(art)
        daily[cat] = kept
    return daily, removed
...
def filter_all(cat_dict):
    ...
    # 카테고리 간 중복 제거 (같은 기사가 두 곳에 실리는 것 방지)
    result, cross_removed = dedupe_across_categories(result)
    if cross_removed:
        print(f"\n  [카테고리 간 중복 제거] {len(cross_removed)}건")
        for title, dropped_cat, kept_cat in cross_removed:
            print(f"    · {title[:42]}")
            print(f"        {dropped_cat} 에서 제외 (→ {kept_cat} 에 유지)")
        total["dup"] += len(cross_removed)
        total["kept"] -= len(cross_removed)
        per_cat = {c: len(v) for c, v in result.items()}
